...
(test recur-instance-p-interval-001
  "Check INTERVAL rule."
  (is (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                        (parse-value "FREQ=DAILY;INTERVAL=2" 'recur)
                        (encode-datetime 01 01 2000 00 00 00)))
  (is (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                        (parse-value "FREQ=DAILY;INTERVAL=2" 'recur)
                        (encode-datetime 03 01 2000 00 00 00)))
  (is (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                        (parse-value "FREQ=DAILY;INTERVAL=2" 'recur)
                        (encode-datetime 05 01 2000 00 00 00)))
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=DAILY;INTERVAL=2" 'recur)
                             (encode-datetime 02 01 2000 00 00 00))))
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=DAILY;INTERVAL=2" 'recur)
                             (encode-datetime 04 01 2000 00 00 00))))
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=DAILY;INTERVAL=2" 'recur)
                             (encode-datetime 06 01 2000 00 00 00)))))
...
(test recur-instance-p-interval-002
  ...
  (is (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                        (parse-value "FREQ=WEEKLY;INTERVAL=2" 'recur)
                        (encode-datetime 15 01 2000 00 00 00)))
  (is (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                        (parse-value "FREQ=WEEKLY;INTERVAL=2" 'recur)
                        (encode-datetime 29 01 2000 00 00 00)))
  (is (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                        (parse-value "FREQ=WEEKLY;INTERVAL=2" 'recur)
                        (encode-datetime 12 02 2000 00 00 00)))
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=WEEKLY;INTERVAL=2" 'recur)
                             (encode-datetime 08 01 2000 00 00 00))))
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=WEEKLY;INTERVAL=2" 'recur)
                             (encode-datetime 02 01 2000 00 00 00))))
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=WEEKLY;INTERVAL=2" 'recur)
                             (encode-datetime 22 01 2000 00 00 00))))
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=WEEKLY;INTERVAL=2" 'recur)
                             (encode-datetime 05 02 2000 00 00 00)))))

(test recur-parser-interval-p-003
  "Check intervals"
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=YEARLY;INTERVAL=4;UNTIL=20160101T010101" 'recur)
                             (encode-datetime 01 01 2009 00 00 00))))
  (is (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                        (parse-value "FREQ=MONTHLY;INTERVAL=6;COUNT=18" 'recur)
                        (encode-datetime 01 07 2008 00 00 00))))

(test recur-instance-p-bymonth
  "Check BYMONTH rule."
  (is (recur-instance-p (encode-datetime 1 1 2000 00 00 00)
                        (parse-value "FREQ=DAILY;BYMONTH=1" 'recur)
                        (encode-datetime 01 01 2000 00 00 00)))
  (is (recur-instance-p (encode-datetime 1 1 2000 00 00 00)
                        (parse-value "FREQ=DAILY;BYMONTH=1" 'recur)
                        (encode-datetime 31 01 2000 00 00 00)))
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             (parse-value "FREQ=DAILY;BYMONTH=1" 'recur)
                             (encode-datetime 01 02 2000 00 00 00))))
  (is (recur-instance-p (encode-datetime 1 1 2000 00 00 00)
...
(test recur-parser-complex-p-001
  ...
  (is (not (recur-instance-p (encode-datetime 01 01 2000 00 00 00)
                             ...
                             (encode-datetime 12 12 2012 00 00 00))))
...
(test recur-complex-sign-p-001
  ...
  (is (recur-instance-p (encode-datetime 28 06 2010 00 00 00)
                        (parse-value "FREQ=MONTHLY;INTERVAL=2;BYDAY=-1MO" 'recur)
                        (encode-datetime 30 8 2010 00 00 00)))
  (is (recur-instance-p (encode-datetime 11 06 2010 00 00 00)
                        (parse-value "FREQ=MONTHLY;BYDAY=-1MO,-2TH,-3FR" 'recur)
                        (encode-datetime 17 06 2010 00 00 00)))
  (is (recur-instance-p (encode-datetime 31 12 2010 00 00 00)
                        (parse-value "FREQ=YEARLY;BYDAY=-1MO,-1FR" 'recur)
                        (encode-datetime 29 12 2014 00 00 00))))
