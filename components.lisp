...
;; ;;; Like `define-abstract-component' but it register the component.
...
;;     (let ((vname (getf default-initargs :name not-found)))
;;       (when (eq vname not-found)
;;         (setq vname (string name))
;;         (setf default-initargs (list* :name vname default-initargs)))
;;       `(setf (translate ',vname :component)
;;              (define-abstract-component ,name ,super-components
;;                ,slots
...
;; (defgeneric read-component-body (component stream))
...
;; (defun create-empty-component (component-name)
...
;;     (create-empty-component component-name)))

;; ;;; TODO: More error-checking!
;; (defmethod read-component-body ((component component) stream)
...
;;         do
;;         (let ((subcomp (create-empty-component cl-value)))
;;           (read-component-body subcomp stream)
...
;; (defun read-component-class (type stream)
;;   (let ((component (read-component-header stream)))
;;     (unless (typep component type)
;;       (error "A ~a component was expected." type))
;;     (read-component-body component stream)
;;     component))

;; (defun read-component (stream)
;;   (read-component-class t stream))
...
;;                     (value (format-values values nil)))
